...
def test_formula_procedure_requires_named_workbook_evidence():
    ...
    assert "dependent formula result that changed" in procedure["content"]
    assert "cached value belong to the same target cell" in procedure["content"]
